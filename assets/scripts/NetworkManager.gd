...
var is_online = false # Whether you are currently in a network session (server or client) (indicates server connection status)
var is_server = false # Whether you are the server (host), not just connected as client
...
func start_server():
	...
	is_online = true
	print("NetworkManager successfully started server for puzzle: ", puzzle_id)
	server_started.emit()

# Connect to a server
func join_server(puzzle_id: String = PuzzleVar.default_path) -> bool:
	if is_online:
		print("WARNING: NetworkManager could not join server, there is an existing network session (is_online)")
		return false
	...
	is_online = true
	print("NetworkManager connected to server at ", SERVER_IP, " for puzzle: ", puzzle_id)
	return true

# Disconnect from the current session
func disconnect_from_server():
	if is_server: # server does not need to disconnect from itself (could be expanded for gracefull shutdown of server?)
		return
	if peer != null:
		peer.close()
...
func _on_peer_connected(id):
	...
	if is_server: # If we're the server, send puzzle info to the new client
...
func _on_peer_disconnected(id):
	...
	if connected_players.has(id):
		...
		if is_server: # Update all clients about the player leaving
			rpc("_update_player_list", connected_players)
