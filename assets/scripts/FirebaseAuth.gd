...
func write_last_login_time() -> void:
	if(NetworkManager.is_server): # server has no need to update last login time as it should be always active
		return
	if _is_writing:
		return
	_is_writing = true
	var user = await get_user_doc(get_box_id())
	user.add_or_update_field("last_login", Time.get_datetime_string_from_system(true, true))
	update_user(user)
	_is_writing = false
